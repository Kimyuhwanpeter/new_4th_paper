...
FLAGS = easydict.EasyDict({"img_size": 512,
                           ...
                           "pre_checkpoint_path": "/content/drive/MyDrive/295",
                           ...
                           "sample_images": "/content/drive/MyDrive/4th_paper/Segmentation/4th_model_ex/sample_images",

                           "save_checkpoint": "/content/drive/MyDrive/4th_paper/Segmentation/4th_model_ex/checkpoint",

                           "save_print": "/content/drive/MyDrive/4th_paper/Segmentation/4th_model_ex/train_out.txt",
...
def cal_loss(model, model2, images, labels, objectiness, class_imbal_labels_buf, object_buf, crop_buf, weed_buf):
    ...
    with tf.GradientTape() as tape2: # channel ==> 3
        ...
        if class_imbal_labels_buf[0] > class_imbal_labels_buf[1]:
            loss4 += binary_focal_loss(alpha=0.8)(only_crop_labels, tf.nn.sigmoid(logits[:, 0]))
        ...
        if class_imbal_labels_buf[0] < class_imbal_labels_buf[1]:
            loss5 += binary_focal_loss(alpha=0.8)(only_weed_labels, tf.nn.sigmoid(logits[:, 1]))
        ...
        if class_imbal_labels_buf[0] < class_imbal_labels_buf[1]:
            loss1 = categorical_focal_loss(alpha=[[0.2, 0.8]])(non_background_labels, tf.nn.softmax(crop_weed_logits, -1))
        if class_imbal_labels_buf[0] > class_imbal_labels_buf[1]:
            loss1 = categorical_focal_loss(alpha=[[0.8, 0.2]])(non_background_labels, tf.nn.softmax(crop_weed_logits, -1))
