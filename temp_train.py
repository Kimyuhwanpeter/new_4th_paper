...
def cal_loss(model, images, labels, objectiness, class_im_plain, ignore_label):
    ...
    with tf.GradientTape() as tape:
        ...
        # crop_loss = true_dice_loss(crop_labels, crop_logits) + tf.nn.sigmoid_cross_entropy_with_logits(crop_labels, crop_logits)
        ...
        # non_crop_loss = false_dice_loss(non_crop_labels, non_crop_logits)
        ###########################################
        ...
        # weed_loss = true_dice_loss(weed_labels, weed_logits)
        ...
        # non_weed_loss = false_dice_loss(non_weed_labels, non_weed_logits)
        ###########################################
    ...
    with tf.GradientTape() as tape:
        ...
        dice_loss = crop_dice_loss + weed_dice_loss

    grads2 = tape.gradient(dice_loss, model.trainable_variables)

    g = lambda x, y: [(x[i] * y[i]) for i in range(len(grads))]
    new_grad = g(grads, grads2)

    optim.apply_gradients(zip(new_grad, model.trainable_variables))
